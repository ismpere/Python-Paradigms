...
class Juego:	#Definimos la clase Juego en la que se encontrara todo lo necesario para nuestra interfaz del juego y sus metodos
    ...
    def invertir(self, widget, data=None):
	for i in range(self.filas + 4):
		for j in range(self.columnas + 4):
			eventoMod = self.eventos[i][j]  #mediante el estado del evento
            		if (eventoMod.estado == True):
                		...
                		stark.show()
                		evento.estado = False
                		evento.x = j       #Establece las coordenadas del evento
                		evento.y = i
	...
	cajaMod.pack_start(self.botonDeshacer, gtk.TRUE, gtk.TRUE, 0)
        self.botonDeshacer.show()
	invertir = gtk.Button("Invertir Tablero")
	invertir.connect("clicked", self.invertir)
	cajaMod.pack_start(invertir, gtk.TRUE, gtk.TRUE, 0)
	invertir.show()
	cajaHorizontal.pack_start(cajaMod, gtk.TRUE, gtk.TRUE, 0)
	cajaMod.show()
